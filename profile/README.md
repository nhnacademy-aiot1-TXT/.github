...
### 프로젝트명: Smart Academy - 환경 모니터링 및 장치 제어 시스템
...
*  회원가입 시 Github와 Google을 통해 가입할 수 있습니다.
...
* 웹 애플리케이션을 통해 사용자는 Academy 환경 데이터를 기반으로 시각화하고, 각 시설들을 제어할 수 있습니다.
   * 관리자
      ...
      * 강의실 별로 선택하여 기기를 추가할 수 있습니다.
      * 침입 감지 알림을 제어할 때 시작시간과 종료시간을 설정하고 시작시간 이후부터 종료시간까지 재실이 감지되면 알림을 보내도록 할 수 있습니다.
      * 상세 센서 페이지를 통해, 실시간 센서 정보를 확인하고 필요한 경우 해당 정보의 상세 로그를 확인할 수 있습니다.
   * 유저
...
* Backend 개발:
   ...
   * Eureka
      ...
      * 프로젝트 상태 모니터링
   * Rule-Engine      
      * AI 모드, 커스텀 모드 장치 설정 수정
      * 센서 데이터를 받아 influxdb 저장하기
      * 재실 센서에서 데이터 받아 침입 감지 설정

   * Device-Control
      * 계정에 따른 알림 리스트 생성
      * 조회 요청에 따라 알림 전송 기능 구현
...
* AI 개발:
   ...
   * predict 서버를 이용하여 예측치를 통해 학습 환경 자동 제어
      * 학습된 AI 모델을 통해 예측치 조정 
      * AI 모델 추가 가능
      * mqtt url과 topic 추가하고 메시지 추가 설정 가능
